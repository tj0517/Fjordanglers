...
export default async function NewInquiryPage() {
  ...
  const guideIds = [...new Set(((expPages ?? []) as Array<{ id: string; experience_name: string; guide_id: string | null }>).map(e => e.guide_id).filter(Boolean))] as string[]
  ...
  const trips = ((expPages ?? []) as Array<{ id: string; experience_name: string; guide_id: string | null }>).map(e => ({
    id:         e.id,
    title:      e.experience_name,

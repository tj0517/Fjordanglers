...
export async function fetchGeoExpPages(): Promise<ExpPage[]> {
  ...
  const { data, error } = await svc
    ...
    .select('id, slug, experience_name, country, region, price_from, price_type, currency, hero_image_url, gallery_image_urls, difficulty, technique, target_species, non_angler_friendly, location_lat, location_lng, location_area, location_spots')
    .eq('status', 'active')

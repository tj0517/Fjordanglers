...
| O-13 | `GclidCapture` stores `fa_utm` (and gclid) to localStorage regardless of cookie banner consent — does this comply with PT art. 173 (Polish Telecommunications Act)? | `src/components/analytics/GclidCapture.tsx`, `src/lib/utm.ts`, `src/lib/gclid.ts`. Raised in FA-0.15: UTM data from localStorage was intentionally excluded from `web_events` to avoid this question. No code changes to those files. | n/a | open — owner: tj |
| O-14 | Nazwy statusów §4 (`new … handed_over`) i mapowanie legacy §4.1 | Zmiana po migracji = kolejna migracja danych. | stage 1, FA-1.03 | **resolved 2026-09-16 (tj)**: nazwy i mapowanie jak w §4/§4.1; `handed_over` jest statusem |
| O-15 | Stripe payment link z aplikacji (Payment Link API, `metadata.inquiry_id`) czy ręcznie w dashboardzie? | Bez metadanych webhook nie dopasuje wpłaty; ręczny link ginie w biegu. | FA-1.12 | **resolved 2026-09-16 (tj)**: z aplikacji; `UnmatchedLinker` zostaje jako ścieżka awaryjna |
| O-16 | Instagram DM: konto firmowe FA w aplikacji Meta + app review — kto, kiedy? | Adapter powstaje w FA-1.13, działa po review; do tego czasu przewodnicy z IG → WhatsApp. | FA-1.13 (nie blokuje) | open — owner: tj |

...
effort: high
agent: fa-core
branch: stage-1/messages-thread
depends_on: [FA-1.03]
blocked_by_questions: [O-15]
touches_db: true
touches_prod: false
estimate_h: 14
owner: tj
...
- `docs/01-architecture.md` §3a (messages), §4 (statusy, „wysyłka może zaproponować przejście")
...
- [ ] UI wątku na `admin/inquiries/[id]`: lista, kompozytor (kanał, rozmówca, treść), przyciski oznaczeń: „oferta od przewodnika" (cena, termin → `guide.offer_received`), „przedstawia ofertę" (`offer.presented` + propozycja `offer_presented`), „klient akceptuje/odrzuca" (`offer.accepted`/`declined` + propozycja statusu), „poinformowano przewodnika o wpłacie", „kontakty wymienione" (+ `handed_over`). Propozycja statusu = checkbox zaznaczony domyślnie, w tym samym submit.
...
- [ ] Testy: `sendMessage` zapisuje wiersz i dokładnie jedno zdarzenie; inbound bez dopasowania trafia do `unmatched`; oznaczenie „przedstawia ofertę" bez wiadomości jest odrzucone; duplikat `external_id` odrzucony.
...
- [ ] `supabase db diff` pusty; typy zawierają `messages`, nie zawierają `lead_messages`.
